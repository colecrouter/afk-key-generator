...
try {
    ...
    if (strpos($p, "Registration") != FALSE) {
		echo "Unable to login";
	}
	else {
		echo "Success";/*
		$ol = file_get_contents("list/acronis/" . $_POST["gcid"] . ".txt");
		foreach ($ol as &$o) {
			if (json_decode($o)[1] == $_POST["data"]["keys"]) {
				$o = json_decode($o);
				$o[3] = FALSE;
				$o = json_encode($o);				
			}
		...
		$ol = implode("\n", $ol);
		$o[3] = FALSE;
		file_put_contents("list/acronis/" . $_POST["gcid"] . ".txt", $ol);*/
	}
